...
import { difficultyLevels, equipmentTypes, exerciseCategories, muscleGroups } from '../types/workout';
export default function Explore() {
    ...
    const filtered = useMemo(() => {
        ...
        return allExercises.filter((item) => (!query || item.name
            .toLocaleLowerCase()
            .normalize('NFD')
            .replace(/\p{Diacritic}/gu, '')
            .includes(query))
    ...
    const filterGroups = [
        { label: 'GRUPO MUSCULAR', values: muscleGroups, selected: muscleFilter, onSelect: (value) => setMuscleFilter(value) },
        { label: 'CATEGORÍA', values: exerciseCategories, selected: categoryFilter, onSelect: (value) => setCategoryFilter(value) },
        { label: 'EQUIPAMIENTO', values: equipmentTypes, selected: equipmentFilter, onSelect: (value) => setEquipmentFilter(value) },
        { label: 'DIFICULTAD', values: difficultyLevels, selected: difficultyFilter, onSelect: (value) => setDifficultyFilter(value) },
    ];
